...
public class SettingOffsetConsumer
{
	public static void main(String args[])
	{
		SettingOffsetConsumer example = new SettingOffsetConsumer();
		long maxReads = 1;
		String topic = "zzz";
		int partition = 0;
		List<String> seeds = Collections.singletonList("localhost");  //"10.28.72.244"
			...
			int a_port) throws Exception
	{
		...
		SimpleConsumer consumer = new SimpleConsumer(leadBroker, a_port, 100000, 64 * 1024,
				clientName);
		// Setting the starting point for the offset.
		long readOffset = 2;// (The getLastOffset is the original method, that
							// will get the offset value considering the
							// previous value.)
//		getLastOffset(consumer, a_topic, a_partition,
				
		Calendar cal = Calendar.getInstance(); 
	    cal.setTime(new Date()); 
	    cal.roll(Calendar.HOUR_OF_DAY, false); // roll one hour back
	    long whichTime = cal.getTime().getTime(); 
		
		getLastOffset(consumer, a_topic, a_partition, whichTime, clientName);		
		...
		while (a_maxReads > 0)
		{
			...
			for (MessageAndOffset messageAndOffset : fetchResponse.messageSet(a_topic, a_partition))
			{
				...
				// System.out.println("Byte Buffer: "+payload);
				byte[] bytes = new byte[payload.limit()];
				// System.out.println("BYTES ARRAY : "+bytes[1]);
				payload.get(bytes);
				// payload.get
				System.out.println(String.valueOf(messageAndOffset.offset()) + ": "
						+ new String(bytes, "UTF-8"));
				// System.out.println("Partition Value : "+a_partition);
				numRead++;
				a_maxReads--;
			}
