...
public class LowLevelConsumer
{
	public static void main(String args[])
	{
		...
		List<String> seedHosts = new ArrayList<String>();
		...
		try
		{
			firstComsumer.run(maxReads, topic, partition, seedHosts, port);
		}
		catch (Exception e)
		{
			...
			int a_port) throws Exception
	{
		...
		String clientName = m_consNamePref + "_Client_" + a_topic + "_" + a_partition;
		...
		while (a_maxReads > 0)
		{
			...
			if (fetchResponse.hasError())
			{
				...
				System.out.println("Error fetching data from the Broker:" + leadBroker
						+ " Reason: " + code);
				if (numErrors > NUM_OF_ERRORS_ALLOWED)
					break;
			...
			int a_partition)
	{
		...
		loop: for (String host : a_seedBrokers)
		{
